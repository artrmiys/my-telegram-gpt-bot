...
async def describe_image(file_id):
    ...
    r = openai.ChatCompletion.create(
        ...
        messages=[
            {"role": "system", "content": "Ты — Ботэнский 🤖. Отвечай уверенно, тепло, без мата и токсичности."},
            {
                "role": "user",
                "content": [
                    {
                    ...
                    {
                        "type": "text",
                        "text": "Опиши атмосферу, эмоции людей и их отношение. Формат: 2 строки реакции + строка Оценка <число>/10 — характеристика."
                    }
                ]
            }
        ]
